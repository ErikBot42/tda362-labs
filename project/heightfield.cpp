...
HeightField::HeightField(void)
    : m_meshResolution(0), m_vao(UINT32_MAX), m_positionBuffer(UINT32_MAX),
      m_uvBuffer(UINT32_MAX), m_indexBuffer(UINT32_MAX), m_numIndices(0),
      m_texid_hf(UINT32_MAX), m_texid_diffuse(UINT32_MAX),
...
void HeightField::loadHeightField(const std::string &heigtFieldPath) {
    ...
    float *data =
        stbi_loadf(heigtFieldPath.c_str(), &width, &height, &components, 1);
    if (data == nullptr) {
...
void HeightField::loadDiffuseTexture(const std::string &diffusePath) {
    ...
    stbi_set_flip_vertically_on_load(true);
    uint8_t *data =
        stbi_load(diffusePath.c_str(), &width, &height, &components, 3);
    if (data == nullptr) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, width, height, 0, GL_RGB,
                 GL_UNSIGNED_BYTE, data); // plain RGB
