...
FboInfo::FboInfo(int w, int h) : FboInfo() { resize(w, h); };
...
void FboInfo::resize(int w, int h) {
    ...
    if (!isComplete) {
        ...
        // bind the texture as color attachment 0 (to the currently bound
        // framebuffer)
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                               GL_TEXTURE_2D, colorTextureTarget, 0);
        glDrawBuffer(GL_COLOR_ATTACHMENT0);

        // bind the texture as depth attachment (to the currently bound
        // framebuffer)
