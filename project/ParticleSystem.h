...
class ParticleSystem {
  public:
    /// Allocates the gpu buffer to hold up to `capacity` particles and the
    /// corresponding vao
    ...
    /// Creates a new particle if there less than `max_size` elements in the
    /// particles array buffer
    void spawn(Particle particle);

    /// Updates all the particles' positions depending on their speed, their
    /// lifetimes, and kills any that are past their life_length
    void process_particles(float dt);

    /// Updates the vertex buffer with the current particle properties, and
    /// renders them
