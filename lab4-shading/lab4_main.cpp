...
void changeScene(std::string sceneName) {
    ...
    selectedMaterialIdx = scenes[currentScene]
                              .models[selectedModelIdxInScene]
                              ->m_meshes[selectedMeshInModel]
                              .m_material_idx;
...
void loadShaders(bool is_reload) {
    GLuint shader =
        labhelper::loadShaderProgram("../lab4-shading/shading.vert",
                                     "../lab4-shading/shading.frag", is_reload);
    if (shader != 0)
        shaderProgram = shader;

    shader = labhelper::loadShaderProgram("../lab4-shading/background.vert",
                                          "../lab4-shading/background.frag",
                                          is_reload);
    if (shader != 0)
        backgroundProgram = shader;

    shader =
        labhelper::loadShaderProgram("../lab4-shading/simple.vert",
                                     "../lab4-shading/simple.frag", is_reload);
    if (shader != 0)
...
void initialize() {
    ...
    struct HDRImage {
        ...
        HDRImage(const string &filename) {
            ...
            data =
                stbi_loadf(filename.c_str(), &width, &height, &components, 3);
            if (data == NULL) {
    ...
    { // Irradiance map
        HDRImage image("../scenes/envmaps/" + envmap_base_name +
                       "_irradiance.hdr");
    ...
    { // Reflection map
        ...
        // We call this again because AMD drivers have some weird issue in the
        // GenerateMipmap function that breaks the first level of the image.
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, image.width, image.height, 0,
                     GL_RGB, GL_FLOAT, image.data);

        const int roughnesses = 8;
        for (int i = 1; i < roughnesses; i++) {
            HDRImage image("../scenes/envmaps/" + envmap_base_name + "_dl_" +
                           std::to_string(i) + ".hdr");
            glTexImage2D(GL_TEXTURE_2D, i, GL_RGB32F, image.width, image.height,
                         0, GL_RGB, GL_FLOAT, image.data);
        }
    }
    ...
    // You can find the valid values for this in `loadScenes`: "Ship", "Material
    // Test" and "Cube"
    changeScene("Ship");
...
void display(void) {
    ...
    mat4 viewMatrix, projectionMatrix;
    {
        ...
        mat3 cameraBaseVectorsWorldSpace(cameraRight, cameraUp,
                                         -camera.direction);
    ...
    lightPosition = vec3(rotate(lightRotation + float(M_PI / 4), worldUp) *
                         lightStartPosition);
    ...
    labhelper::setUniformSlow(shaderProgram, "modelViewProjectionMatrix",
                              projectionMatrix * viewMatrix * modelMatrix);
    labhelper::setUniformSlow(shaderProgram, "viewInverse",
                              inverse(viewMatrix));
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_MOUSEMOTION) {
            ...
            if (g_isMouseDragging) {
                ...
                mat4 pitch =
                    rotate(rotation_speed * -delta_y,
                           normalize(cross(camera.direction, worldUp)));
                camera.direction =
                    vec3(pitch * yaw * vec4(camera.direction, 0.0f));
...
void gui() {
    ...
    labhelper::Model *selected_model =
        scenes[currentScene].models[selectedModelIdxInScene];

    ImGui::Begin("Control Panel", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
    {
        if (ImGui::CollapsingHeader("Meshes", "meshes_ch", true, true)) {
            if (ImGui::ListBox("Meshes", &selectedMeshInModel, mesh_getter,
                               (void *)&selected_model->m_meshes,
                               int(selected_model->m_meshes.size()), 6)) {
                selectedMaterialIdx =
                    selected_model->m_meshes[selectedMeshInModel]
                        .m_material_idx;
            }
        ...
        if (ImGui::CollapsingHeader("Material", "materials_ch", true, true)) {
            labhelper::Material &material =
                selected_model->m_materials[selectedMaterialIdx];
            ...
            ImGui::SliderFloat("Shininess", &material.m_shininess, 0.0f,
                               5000.0f, "%.3f", 2);
        ...
        if (ImGui::CollapsingHeader("Light sources", "lights_ch", true, true)) {
            ImGui::SliderFloat("Environment multiplier",
                               &environment_multiplier, 0.0f, 10.0f);
            ImGui::ColorEdit3("Point light color", &point_light_color.x);
            ImGui::SliderFloat("Point light intensity multiplier",
                               &point_light_intensity_multiplier, 0.0f,
                               10000.0f, "%.3f", 2.f);
            ImGui::Checkbox("Manual light only (right-click drag to move)",
                            &lightManualOnly);
        }
