    ...
    class tempTexHelper {
      public:
        static void loadCubeMapFace(std::string filename, GLenum face) {
            ...
            glTexImage2D(face, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         image);
        }
    };
...
bool checkGLError(const char *file, int line) {
    ...
    for (GLenum glErr = glGetError(); glErr != GL_NO_ERROR;
        ...
        if (!sError) {
            sError =
                reinterpret_cast<const GLubyte *>(" (no message available)");
        }
...
void fatal_error(std::string errorString, std::string title) {
    ...
    // abort()
#if defined(_WIN32)
    MessageBox(0, errorString.c_str(), title.c_str(),
