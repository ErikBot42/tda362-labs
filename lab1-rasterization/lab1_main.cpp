...
void initialize() {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 labhelper::array_length(positions) * sizeof(float), positions,
                 GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 labhelper::array_length(colors) * sizeof(float), colors,
                 GL_STATIC_DRAW);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);
    // Attaches positionBuffer to vertexArrayObject, in the 0th attribute
    // location
    glVertexAttribPointer(0, 3, GL_FLOAT, false /*normalized*/, 0 /*stride*/,
                          0 /*offset*/);
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, false /*normalized*/, 0 /*stride*/,
                          0 /*offset*/);
    ...
    // See OpenGL spec §2.20
    // http://www.cse.chalmers.se/edu/course/TDA361/glspec30.20080923.pdf#page=104&zoom=75
    ...
    // Compile the shader, translates into internal representation and checks
    // for errors.
    ...
    // Create a program object and attach the two shaders we have compiled, the
    // program object contains both vertex and fragment shaders as well as
    // information about uniforms and attributes common to both.
    shaderProgram = glCreateProgram();
    glAttachShader(shaderProgram, fragmentShader);
    glAttachShader(shaderProgram, vertexShader);

    // Now that the fragment and vertex shader has been attached, we no longer
    // need these two separate objects and should delete them. The attachment to
    // the shader program will keep them alive, as long as we keep the
    // shaderProgram.
    glDeleteShader(vertexShader);
    glDeleteShader(fragmentShader);

    // Link the different shaders that are bound to this program, this creates a
    // final shader that we can use to render geometry with.
    glLinkProgram(shaderProgram);

    // Check for linker errors, many errors, such as mismatched in and out
    // variables between vertex/fragment shaders,  do not appear before linking.
    {
        GLint linkOk = 0;
...
void display(void) {
    ...
    glClearColor(g_clearColor[0], g_clearColor[1], g_clearColor[2],
                 1.0);  // Set clear color
    glClear(GL_BUFFER); // Clears the color buffer and the z-buffer
                        // Instead of glClear(GL_BUFFER) the call should be
                        // glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)

    // We disable backface culling for this tutorial, otherwise care must be
    // taken with the winding order of the vertices. It is however a lot faster
    // to enable culling when drawing large scenes.
    glDisable(GL_CULL_FACE);

    // Set the Shader Program to use
    glUseProgram(
        shaderProgram); // Set the shader program to use for this draw call

    // Task 5: Set the `triangleColor` uniform in the shader to
    // `g_triangleColor`
    ...
    glUseProgram(
        0); // "unsets" the current shader program. Not really necessary.
}
...
void gui() {
    ...
    // Task 5: Add a new ColorEdit3 control to modify the g_triangleColor
    // variable
