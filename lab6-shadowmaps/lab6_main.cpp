...
// Shadow map
///////////////////////////////////////////////////////////////////////////////
enum ClampMode { Edge = 1, Border = 2 };
...
void initialize() {
    ...
    if (_initialized++) {
        labhelper::fatal_error(
            "You must not call the initialize() function more than once!",
            "Already initialized!");
    }
    ...
    ///////////////////////////////////////////////////////////////////////
    backgroundProgram =
        labhelper::loadShaderProgram("../lab6-shadowmaps/background.vert",
                                     "../lab6-shadowmaps/background.frag");
    shaderProgram = labhelper::loadShaderProgram(
        "../lab6-shadowmaps/shading.vert", "../lab6-shadowmaps/shading.frag");
    depthProgram = labhelper::loadShaderProgram(
        "../lab6-shadowmaps/depth.vert", "../lab6-shadowmaps/depth.frag");
    simpleShaderProgram = labhelper::loadShaderProgram(
        "../lab6-shadowmaps/simple.vert", "../lab6-shadowmaps/simple.frag");
...
void drawScene(GLuint currentShaderProgram, const mat4 &viewMatrix,
               const mat4 &projectionMatrix, const mat4 &lightViewMatrix,
               const mat4 &lightProjectionMatrix) {
    glUseProgram(currentShaderProgram);
    // Light source
    vec4 viewSpaceLightPosition = viewMatrix * vec4(lightPosition, 1.0f);
    labhelper::setUniformSlow(currentShaderProgram, "point_light_color",
                              point_light_color);
    labhelper::setUniformSlow(currentShaderProgram,
                              "point_light_intensity_multiplier",
                              point_light_intensity_multiplier);
    labhelper::setUniformSlow(currentShaderProgram, "viewSpaceLightPosition",
                              vec3(viewSpaceLightPosition));
    labhelper::setUniformSlow(
        currentShaderProgram, "viewSpaceLightDir",
        normalize(vec3(viewMatrix * vec4(-lightPosition, 0.0f))));
    labhelper::setUniformSlow(currentShaderProgram, "spotOuterAngle",
                              std::cos(radians(outerSpotlightAngle)));
...
void display(void) {
    ...
    ///////////////////////////////////////////////////////////////////////////
    mat4 projMatrix =
        perspective(radians(45.0f), float(w) / float(h), 5.0f, 500.0f);
    mat4 viewMatrix =
        lookAt(camera.position, camera.position + camera.direction, worldUp);

    mat3 lightRot = rotate(radians(lightAzimuth), vec3(0, 1, 0)) *
                    rotate(radians(lightZenith), vec3(0, 0, 1));
    lightPosition = lightRot * vec3(lightDistance, 0, 0);
    mat4 lightViewMatrix =
        lookAt(lightPosition, vec3(0.0f), lightRot * worldUp);
    ...
    drawScene(shaderProgram, viewMatrix, projMatrix, lightViewMatrix,
              lightProjMatrix);
...
void gui() {
    ...
    ImGui::RadioButton("Clamp to border", &shadowMapClampMode,
                       ClampMode::Border);
