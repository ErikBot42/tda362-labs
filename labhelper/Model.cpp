...
bool Texture::load(const std::string &_directory, const std::string &_filename,
                   int _components) {
    filename = file::normalise(_filename);
    directory = file::normalise(_directory);
    valid = true;
    int components;
    data = stbi_load((directory + filename).c_str(), &width, &height,
                     &components, _components);
    if (data == nullptr) {
        std::cout << "ERROR: loadModelFromOBJ(): Failed to load texture: "
    ...
    if (_components == 1) {
        ...
        std::cout << "Texture loading not implemented for this number of "
                     "compenents.\n";
        exit(1);
    }
...
glm::vec4 Texture::sample(glm::vec2 uv) const {
    ...
    if (n_components == 4) {
        return glm::vec4(data[(y * width + x) * 4 + 0],
                         data[(y * width + x) * 4 + 1],
                         data[(y * width + x) * 4 + 2],
                         data[(y * width + x) * 4 + 3]) /
               255.f;
        ...
        return glm::vec4(data[(y * width + x) * n_components + 0],
                         data[(y * width + x) * n_components + 0],
                         data[(y * width + x) * n_components + 0],
                         data[(y * width + x) * n_components + 0]) /
               255.f;
    }
...
Model *loadModelFromOBJ(std::string path) {
    ...
    if (extension != ".obj") {
        std::cout << "Fatal: loadModelFromOBJ(): Expecting filename ending in "
                     "'.obj'\n";
        exit(1);
    }
    ...
    bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &err,
                                (directory + filename + extension).c_str(),
    ...
    for (const auto &m : materials) {
        ...
        if (m.roughness_texname != "") {
            material.m_shininess_texture.load(directory, m.roughness_texname,
                                              1);
        }
        material.m_emission =
    ...
    for (const auto &shape : shapes) {
        for (int face = 0; face < int(shape.mesh.indices.size()) / 3; face++) {
            glm::vec3 v0 = glm::vec3(
                attrib.vertices[shape.mesh.indices[face * 3 + 0].vertex_index *
                                    3 +
                                0],
                attrib.vertices[shape.mesh.indices[face * 3 + 0].vertex_index *
                                    3 +
            ...
            glm::vec3 v1 = glm::vec3(
                attrib.vertices[shape.mesh.indices[face * 3 + 1].vertex_index *
                                    3 +
                                0],
                attrib.vertices[shape.mesh.indices[face * 3 + 1].vertex_index *
                                    3 +
            ...
            glm::vec3 v2 = glm::vec3(
                attrib.vertices[shape.mesh.indices[face * 3 + 2].vertex_index *
                                    3 +
                                0],
                attrib.vertices[shape.mesh.indices[face * 3 + 2].vertex_index *
                                    3 +
            ...
            auto_normals[shape.mesh.indices[face * 3 + 0].vertex_index] +=
                glm::vec4(face_normal, 1.0f);
            auto_normals[shape.mesh.indices[face * 3 + 1].vertex_index] +=
                glm::vec4(face_normal, 1.0f);
            auto_normals[shape.mesh.indices[face * 3 + 2].vertex_index] +=
                glm::vec4(face_normal, 1.0f);
        }
    }
    ...
    for (int s = 0; s < shapes.size(); ++s) {
        ...
        while (next_material_index != -1) {
            ...
            Mesh mesh;
            mesh.m_name =
                shape.name + "_" + materials[current_material_index].name;
            ...
            for (int i = current_material_starting_face; i < number_of_faces;
                 i++) {
                if (shape.mesh.material_ids[i] != current_material_index) {
                    ...
                    for (int j = 0; j < 3; j++) {
                        ...
                        model->m_positions[vertices_so_far + j] = glm::vec3(
                            attrib.vertices[shape.mesh.indices[i * 3 + j]
                                                    .vertex_index *
                                                3 +
                                            0],
                            attrib.vertices[shape.mesh.indices[i * 3 + j]
                        ...
                        if (shape.mesh.indices[i * 3 + j].normal_index == -1) {
                            ...
                            model->m_normals[vertices_so_far + j] = glm::vec3(
                                auto_normals[shape.mesh.indices[i * 3 + j]
                                                 .vertex_index]);
                        } else {
                            model->m_normals[vertices_so_far + j] = glm::vec3(
                                attrib.normals[shape.mesh.indices[i * 3 + j]
                                                       .normal_index *
                                                   3 +
                                               0],
                                attrib.normals[shape.mesh.indices[i * 3 + j]
                        ...
                        if (shape.mesh.indices[i * 3 + j].texcoord_index ==
                            -1) {
                            // No UV coordinates. Use null.
                            model->m_texture_coordinates[vertices_so_far + j] =
                                glm::vec2(0.0f);
                        } else {
                            model->m_texture_coordinates[vertices_so_far + j] =
                                glm::vec2(
                                    attrib
                                        .texcoords[shape.mesh.indices[i * 3 + j]
                                                           .texcoord_index *
                                                       2 +
                                                   0],
                                    attrib
        ...
        if (number_of_materials_in_shape == 1) {
            // If there's only one material, we don't need the material name in
            // the mesh name
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 model->m_texture_coordinates.size() * sizeof(glm::vec2),
...
void saveModelMaterialsToMTL(Model *model, std::string filename) {
    ...
    for (const auto &mat : model->m_materials) {
        ...
        mat_file << "Kd " << mat.m_color.x << " " << mat.m_color.y << " "
                 << mat.m_color.z << "\n";
        mat_file << "Ks " << mat.m_fresnel << " " << mat.m_fresnel << " "
                 << mat.m_fresnel << "\n";
        mat_file << "Pm " << mat.m_metalness << "\n";
        mat_file << "Pr " << mat.m_shininess << "\n";
        mat_file << "Ke " << mat.m_emission.x << " " << mat.m_emission.y << " "
                 << mat.m_emission.z << "\n";
...
void saveModelToOBJ(Model *model, std::string filename) {
    ...
    for (auto mesh : model->m_meshes) {
        ...
        obj_file << "usemtl " << model->m_materials[mesh.m_material_idx].m_name
                 << "\n";
        for (uint32_t i = mesh.m_start_index;
             i < mesh.m_start_index + mesh.m_number_of_vertices; i++) {
        ...
        for (uint32_t i = mesh.m_start_index;
            ...
            obj_file << "vn " << model->m_normals[i].x << " "
                     << model->m_normals[i].y << " " << model->m_normals[i].z
                     << "\n";
        }
        for (uint32_t i = mesh.m_start_index;
             i < mesh.m_start_index + mesh.m_number_of_vertices; i++) {
        ...
        for (int i = 0; i < number_of_faces; i++) {
            obj_file << "f " << vertex_counter << "/" << vertex_counter << "/"
...
void render(const Model *model, const bool submitMaterials) {
    ...
    for (auto &mesh : model->m_meshes) {
        if (submitMaterials) {
            ...
            material.m_metalness_texture.gl_id );
            }
            if ( has_fresnel_texture )
            {
                    glActiveTexture( GL_TEXTURE3 );
                    glBindTexture( GL_TEXTURE_2D,
            material.m_fresnel_texture.gl_id );
            }
            if ( has_shininess_texture )
            {
                    glActiveTexture( GL_TEXTURE4 );
                    glBindTexture( GL_TEXTURE_2D,
            material.m_shininess_texture.gl_id );
            }
            */
            ...
            setUniformSlow(current_program, "has_color_texture",
                           has_color_texture);
            setUniformSlow(current_program, "has_emission_texture",
                           has_emission_texture);

            setUniformSlow(current_program, "material_color", material.m_color);
            setUniformSlow(current_program, "material_metalness",
                           material.m_metalness);
            setUniformSlow(current_program, "material_fresnel",
                           material.m_fresnel);
            setUniformSlow(current_program, "material_shininess",
                           material.m_shininess);
            setUniformSlow(current_program, "material_emission",
                           material.m_emission);

            // Actually unused in the labs
            /*
            setUniformSlow( current_program, "has_metalness_texture",
            has_metalness_texture ); setUniformSlow( current_program,
            "has_fresnel_texture", has_fresnel_texture ); setUniformSlow(
            current_program, "has_shininess_texture", has_shininess_texture );
            */
        }
        glDrawArrays(GL_TRIANGLES, mesh.m_start_index,
