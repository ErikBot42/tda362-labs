...
void initialize() {
    ...
    // Load shader program
    shaderProgram = labhelper::loadShaderProgram("../lab3-camera/simple.vert",
                                                 "../lab3-camera/simple.frag");
...
void display() {
    ...
    glClear(GL_COLOR_BUFFER_BIT |
            GL_DEPTH_BUFFER_BIT); // Clears the color buffer and the z-buffer
    ...
    mat4 constantViewMatrix =
        mat4(0.707106769f, -0.408248276f, 1.00000000f, 0.000000000f,  //
    ...
    mat4 projectionMatrix = perspective(
        radians(pp.fov), float(pp.w) / float(pp.h), pp.near, pp.far);

    const int mvploc =
        glGetUniformLocation(shaderProgram, "modelViewProjectionMatrix");
    const int mloc = glGetUniformLocation(shaderProgram, "modelMatrix");

    // Concatenate the three matrices and pass the final transform to the vertex
    // shader

    // City
    mat4 modelViewProjectionMatrix =
        projectionMatrix * viewMatrix * cityModelMatrix;
